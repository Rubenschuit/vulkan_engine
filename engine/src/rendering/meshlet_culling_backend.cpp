...
MeshletCullingBackend::MeshletCullingBackend(MeshletCullingSystem& meshlet,
	...
	: m_meshlet{meshlet}, m_gpu_cull{gpu_cull} {}
...
void MeshletCullingBackend::cull(VeFrameInfo& fi, GpuSceneManager& gpu_scene) {
	...
	m_meshlet.dispatch(cmd, fi, gpu_scene);
}
...
void MeshletCullingBackend::renderDepthPrePass(VeFrameInfo& fi, PbrMegaBuffer& mega,
	...
	dps.renderGpuCulledMeshlets(fi, mega,
		m_meshlet.getMeshletIndirectBuffer(fi.current_frame),
		m_meshlet.getMeshletDrawCounts(fi.current_frame),
		m_meshlet.getCpuDrawCounts());
}
...
void MeshletCullingBackend::renderShadows(VeFrameInfo& fi, ShadowRenderSystem& srs,
	...
	if (m_gpu_shadow_fallback)
		...
		srs.renderShadowMapsGpuCulledMeshlets(fi, m_meshlet, mega, gpu_scene);
}
...
void MeshletCullingBackend::renderTransparency(VeFrameInfo& fi, PbrRenderSystem& pbr,
	...
	auto& indirect = m_meshlet.getMeshletIndirectBuffer(frame);
	auto& draw_counts = m_meshlet.getMeshletDrawCounts(frame);
	const uint32_t* cpu_counts = m_meshlet.getCpuDrawCounts();
...
vk::raii::DescriptorSet& MeshletCullingBackend::getGlobalDescriptorSet(uint32_t frame) {
	return m_meshlet.getGlobalDescriptorSet(frame);
}
...
void MeshletCullingBackend::collectStats(uint32_t frame, UIContext& ui, Registry&) const {
	...
	if (rb) {
		...
		// drawIndirectCount supported: no CPU readback
		ui.stats.draw_calls = 0;
		ui.stats.cull_visible_objects = 0;
		ui.stats.transparent_draw_calls = 0;
	}
	ui.stats.visible_triangles = 0; // meshlet system doesn't track triangle counts yet
}

void MeshletCullingBackend::setHizEnabled(bool enabled) { m_meshlet.setHizEnabled(enabled); }
bool MeshletCullingBackend::isHizEnabled() const { return m_meshlet.isHizEnabled(); }
